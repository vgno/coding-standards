...
A three letter prefix (e.g. `VGN`) should always be used for class names and constants, however may be omitted for Core Data entity names. Constants should be camel-case with all words capitalized and prefixed by the related class name for clarity.

**For example:**

```objc
static const NSTimeInterval VGNArticleViewControllerNavigationFadeAnimationDuration = 0.3;
```
...
```objc
static NSString * const VGNAboutViewControllerCompanyName = @"VG";

static const CGFloat VGNImageThumbnailHeight = 50.0;
```

**Not:**

```objc
#define CompanyName @"VG"
...
```objc
typedef NS_ENUM(NSInteger, VGNAdRequestState) {
    VGNAdRequestStateInactive,
    VGNAdRequestStateLoading
};
```
...
```objc
typedef NS_OPTIONS(NSUInteger, VGNAdCategory) {
  VGNAdCategoryAutos      = 1 << 0,
  VGNAdCategoryJobs       = 1 << 1,
  VGNAdCategoryRealState  = 1 << 2,
  VGNAdCategoryTechnology = 1 << 3
};
```

## Private Properties

Private properties should be declared in class extensions (anonymous categories) in the implementation file of a class. Named categories (such as `VGNPrivate` or `private`) should never be used unless extending another class.

**For example:**

```objc
@interface VGNAdvertisement ()
...
Complex if statements should be assigned ta a meaningfull variable, it allows easier maintenance.
